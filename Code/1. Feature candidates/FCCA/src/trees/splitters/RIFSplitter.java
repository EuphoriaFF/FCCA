...
public class RIFSplitter implements NodeSplitter{
	...
	public RIF getRif_transformer() {
		return rif_transformer;
	}

	protected RIF rif_transformer;
	public RifFilters filter_type;

	public GenericGiniSplitter getBinary_splitter() {
		return binary_splitter;
	}
	...
	public TIntObjectMap<TSDataset> train(TSDataset sample, int[] indices) throws Exception {
		...
		if (AppContext.rif_transform_level == TransformLevel.Node) {
			rif_transformer = new RIF(this.filter_type, AppContext.rif_num_intervals, AppContext.rif_splitters_per_node / 2);
			rif_transformer.fit(sample);
			trans = rif_transformer.transform(sample);
//			if(Double.isNaN(trans.get_series(0).getData(0))){
...
//			throw new Exception("empty splits found! check for bugs");
	...
	//返回当前时间序列所选的属性
	public void getattr(TimeSeries query, int queryIndex, ArrayList<Double> singledata_mat){
		TimeSeries transformed_query;
		...
		singledata_mat.add(binary_splitter.getattr(transformed_query));

	}

	public double getattr_withVal(TimeSeries query, int queryIndex){
		TimeSeries transformed_query;
		...
		return binary_splitter.getattr(transformed_query);
