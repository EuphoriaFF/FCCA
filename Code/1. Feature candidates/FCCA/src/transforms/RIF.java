...
//	protected int maxLag;	//max lag for each interval is stored in the [][3] 3rd dim of intervals array
	...
	public RifFilters getFilter_type() {
		return filter_type;
	}
	...
	private double[] convert(double[] data, RifFilters filter, int maxLag) {
		...
		}else if(filter==RifFilters.ACF_PS_separately){
			double[] acf = ACF.transformArray(data, maxLag);
			double[] spectral = transformSpectral(data, RifFilters.PS);

			int len = acf.length + spectral.length;

			out = new double[len];	//TODO check

			//new changes in v1.1.17 adding num_attribs_per_component
			System.arraycopy(acf, 0, out, 0, Math.min(num_attribs, acf.length));
			System.arraycopy(spectral, 0, out, acf.length, Math.min(num_attribs,spectral.length));
	...
	public String toString() {
		if (intervals != null) {
//			return "RIF_transform["+num_intervals+ ", filter_type:" + filter_type + "]";
			return "RIF_transform["+intervals.get(0)[0]+","+intervals.get(0)[1]+ ",filter_type:" + filter_type + "]";
		}
