...
public class SFA implements Serializable {
	...
	public static class Words {
		//记录word数组和long值的map 例如：map[ [0,1,2,3] ] = 11100100对应的long
		public static HashMap<Integer, short[]> wordtolong = new HashMap<>();
		...
		public static long createWord(short[] words, int features, byte usedBits) {
			long trans_value = fromByteArrayOne(words, features, usedBits);
//			System.out.println(wordtolong);
//			Words.wordtolong.put(trans_value, words);
			if(!wordtolong.containsKey(trans_value)){
				wordtolong.put((int)trans_value, words);
...
//			return fromByteArrayOne(words, features, usedBits);
	...
	public double[] transformWindowing_getMFT(TimeSeries timeSeries, int st) {
		double[][] mft = this.transformation.transformWindowing(timeSeries, this.maxWordLength);
		return mft[st];
	}

	public double[][] getMFT(TimeSeries timeSeries) {
		double[][] mft = this.transformation.transformWindowing(timeSeries, this.maxWordLength);
	...
	protected double[][] fillOrderline(TimeSeries[] samples, int l) {
		...
		for (int i = 0; i < samples.length; i++) {
			...
			for (int j = 0; j < transformedSamples[i].length; j++) {
				...
				this.orderLine[j].add(new ValueLabel(value, samples[i].getLabel().doubleValue()));	//TODO int to dbl convt
			}
		}
