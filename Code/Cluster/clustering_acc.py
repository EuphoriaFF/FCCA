...
def run_CURE(data, cluster_k, dist, cal_dis_between_clusters):
    ...
    if num_cluster == 1:
        acc_tmp = getFeatureValacc(data[0])
        k_centroids_dict[num_cluster] = clusterInfo([data[0]], [1], [data[0][3]*acc_tmp])
        return k_centroids_dict

    if num_cluster in cluster_k:
        infogain_tmp = []
        for d in data:
            acc_tmp = getFeatureValacc(d)
            infogain_tmp.append(d[3]*acc_tmp)
    ...
    while num_cluster > 1:
        ...
        if num_cluster in cluster_k:
            ...
            for i in range(0, len(clusters)):
                ...
                acc_tmp = getFeatureValacc(clusters[i].center)
                centroids_infogain.append(float(clusters[i].center[3])*acc_tmp)
