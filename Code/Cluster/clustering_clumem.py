...
class clusterInfo:
    ...
    def __init__(self, centroids, clu_num, centroids_infogain, clu_mem):
...
def run_CURE(data, cluster_k, dist, cal_dis_between_clusters):
    ...
    if num_cluster in cluster_k:
        ...
        for d in data:
            ...
            clu_mem.append([d])
        k_centroids_dict[num_cluster] = clusterInfo(data, [1]*len(data), infogain_tmp, clu_mem)
    ...
    while num_cluster > 1:
        ...
        if num_cluster in cluster_k:
            centroids = []  # 聚类中心 [f f f]
            centroids_infogain = []
            clu_num = []  # 聚类个数 [2 1 2]
            clu_mem = []  # 每个聚类中的所有特征 [[f f], [f], [f f]],
            for i in range(0, len(clusters)):
                ...
                clu_mem_k = []
                for clu_mem_id in clusters[i].index:
                    clu_mem_k.append(data[clu_mem_id])
                clu_mem.append(clu_mem_k)
            k_centroids_dict[num_cluster] = clusterInfo(centroids, clu_num, centroids_infogain, clu_mem)
